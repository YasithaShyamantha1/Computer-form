...
<body>
		...
		<div class="container-fuild">
			...
			<div class="main-container">
				...
				<div class="row row-sm">
					...
					<hr class="mg-y-10">
					<div class="col-lg-3" style="padding-top=20px; background-colour=#ffe6cc;">
    ...
    <?php
    $sql2 = "SELECT * FROM tbl_cashgrant 
             JOIN tbl_asc ON tbl_cashgrant.asc_code = tbl_asc.asc_code 
             JOIN tbl_district ON tbl_asc.dis_id = tbl_district.dis_id 
             JOIN tbl_famer ON tbl_cashgrant.nic_check = tbl_famer.nic_check
             WHERE tbl_cashgrant.nic_check = '$nic_check' 
             AND cg_status = '2' 
             ORDER BY tbl_cashgrant.season DESC";
    $result2 = $conn->query($sql2);
    ?>
    ...
    <table class="table table-responsive">
        ...
        <?php
        $fuel = "SELECT * FROM tbl_subsidies 
                 JOIN tbl_famer ON tbl_famer.f_sn = tbl_subsidies.f_sn 
				 ...
				 JOIN tbl_bankbranch ON tbl_bankbranch.bank_code = tbl_district.bank_code
                 WHERE tbl_subsidies.s_type = 'Fuel' 
                 AND tbl_subsidies.f_sn = '$f_sn';";

        $result5 = $conn->query($fuel);

        while ($cul = mysqli_fetch_assoc($result2)) { ?>
            <tr>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>{$cul['season']}</font>" : $cul['season']; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>Cash Grant</font>" : "Cash Grant"; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>{$cul['dis_name']}</font>" : $cul['dis_name']; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>{$cul['asc_name']}</font>" : $cul['asc_name']; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>{$cul['l_olddos']}</font>" : $cul['l_olddos']; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>{$cul['culti_extent']}</font>" : $cul['culti_extent']; ?></td>
                <td><?php echo ($cul['season'] == "2024/25 Maha") ? "<font color='blue'>Rs. {$cul['amount']}</font>" : "Rs. {$cul['amount']}"; ?></td>
                <td><?php 
                    if ($cul['season'] == "2024/25 Maha") echo "<font color='blue'>";
                    if ($cul['bank_reject'] == "1") {
                        echo "Rejected From Bank";
                    } else {
                        if ($cul['trance_date'] == "" || $cul['trance_date'] == "0000-00-00") {
                            echo ($cul['season'] == "2022/23 Maha") ? "Paid" : "Ready to Pay";
                        } else {
                            echo "Deposit on " . $cul['trance_date'];
                        }
                    }
                    if ($cul['season'] == "2024/25 Maha") echo "</font>";
                ?></td>
                
                <td>
               <?php 
               // Fetch multiple bank details using bank_code and branch_code
   
            echo "<li><b>Bank Code:</b> " . $cul['bank_code'] . 
                 " <br><b>Branch Code:</b> " . $cul['branch_code'] . 
				 "<br><b>Branch Name:</b> " . $cul['branch_name'] . 
                 " <br><b>Account No:</b> " . $cul['acc_no'] . "</li><hr>";
        }
